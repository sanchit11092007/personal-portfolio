(() => {
  ...
  function initCursorTrail() {
    ...
    let lastX = 0, lastY = 0;
    const threshold = 15; // px mouse must move before spawning next particle
    ...
    document.addEventListener("mousemove", (e) => {
      ...
      // Select color based on which section mouse is over
      let color = colors[0];
      const hoveredElement = document.elementFromPoint(e.clientX, e.clientY);
      if (hoveredElement) {
        const section = hoveredElement.closest("section");
        if (section) {
          if (section.id === "skills") color = colors[1];
          else if (section.id === "journey") color = colors[2];
          else if (section.id === "MIKASA-ai") color = colors[3];
        }
      }
  ...
  function initLiveTicker() {
    const lists = $$(".live-ticker-list");
    if (!lists.length) return;

    const learningItems = [
      "Currently Mastering DSA",
      "Building Neural Networks",
      "Optimizing Scikit-Learn Pipelines",
      "Deploying ML Models",
      "Exploring Transformers & LLMs",
      "Sharpening SQL & Data Pipelines",
    ...
    lists.forEach(list => {
      list.innerHTML = "";
      // Triple items to ensure seamless infinite scroll loop
      const repeatCount = 3;
      for (let i = 0; i < repeatCount; i++) {
        learningItems.forEach(item => {
          const li = document.createElement("div");
          li.className = "live-ticker-item";
          li.innerHTML = `<span class="live-ticker-dot"></span><span>${item}</span>`;
          list.appendChild(li);
        });
      }
    });
